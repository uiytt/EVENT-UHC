...
public class ChaosEventWeirdBiomes extends ChaosEvent {
	...
	public ChaosEventWeirdBiomes() {
		super("WeirdBiomes", Material.RED_GLAZED_TERRACOTTA, 10, Type.NORMAL, new String[]{"Des biomes de blocs aléatoires spawn"});
	}
	...
	protected void onEnable() {
		...
		Bukkit.broadcastMessage(ConfigManager.EVENT_HEADER + "Des choses bizzares se passent aux cordonées :");
		weirdBiomes.forEach(biome -> Bukkit.broadcastMessage(ConfigManager.EVENT_HEADER + "X:" + biome.getCenter().getX() + " Z:" + biome.getCenter().getZ()));
